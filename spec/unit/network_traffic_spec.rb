...
module Capybara::Poltergeist
  describe NetworkTraffic do
    ...
    describe "full request/response" do
      before(:all) do
    ...
    describe "request only" do
      before(:all) do
        dir = File.dirname(__FILE__) + "/../support"
        json = File.read("#{dir}/network_traffic_request_only.json")
        @hash = MultiJson.load(json)
        @traffic = NetworkTraffic.new(@hash)
      end

      it "still creates the request object" do
        @traffic.request.url.should match(/test\.js$/)
      end

      it "creates an empty response object" do
        @traffic.response.status.should be_nil
      end

      it "keeps time as nil" do
        @traffic.response.time.should be_nil
      end
    end
