...
Poltergeist.Node = (function() {
  ...
  Node.DELEGATES = ['allText', 'visibleText', 'getAttribute', 'value', 'set', 'setAttribute', 'isObsolete', 'removeAttribute', 'isMultiple', 'select', 'tagName', 'find', 'getAttributes', 'isVisible', 'position', 'trigger', 'parentId', 'parentIds', 'mouseEventTest', 'scrollIntoView', 'isDOMEqual', 'isDisabled', 'deleteText', 'containsSelection', 'getProperty'];

  function Node(_at_page, _at_id) {
    this.page = _at_page;
    this.id = _at_id;
  }
  ...
  Node.prototype.dragBy = function(x, y) {
    ...
    position = this.mouseEventPosition();
    this.page.mouseEvent('mousedown', position.x, position.y);
    return this.page.mouseEvent('mouseup', position.x + x - 5, position.y + y - 5);
  };
