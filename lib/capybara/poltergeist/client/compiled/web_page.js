...
Poltergeist.WebPage = (function() {
  ...
  WebPage.CALLBACKS = ['onConsoleMessage', 'onError', 'onLoadFinished', 'onInitialized', 'onLoadStarted', 'onResourceRequested', 'onResourceRedirect', 'onResourceReceived', 'onResourceError', 'onNavigationRequested', 'onUrlChanged', 'onPageCreated', 'onClosing'];
  ...
  WebPage.prototype.onResourceRedirectNative = function(redirect) {
    var ref2;
    if ((ref2 = this._networkTraffic[redirect.id]) != null) {
      ref2.responseParts.push({
        redirectURL: redirect.url
      });
    }
    if (this.requestId === redirect.id) {
      this.redirectURL = this.normalizeURL(redirect.url);
    }
    return true;
  };
